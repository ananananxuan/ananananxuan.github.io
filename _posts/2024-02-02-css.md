...
无论多少个空格、换行、tab，都会折叠为一个空格如果我们想让img标签之间没有空隙，必须紧密连接
...
overflow清除浮动 在父级元素style中增加overflow：hidden；clear：both
伪对象方式
定位

相对定位

在style中设置position：relative

left：200px  距离左边框200px

top：200px  距离上边框200px

绝对定位

在style中设置position：absolute
...
在style中设置z-index：100，z-index：50
...
在style中添加属性 border-radius：20px/100 %

写两个值，分别代表左上角、右下角和右上角和左下角
...
使元素从一种样式逐渐变化成为另一种样式的效果

规定变化的时间 from to or 0% 100%
...
优点：轻量级别，减少http请求；灵活性，可以使用css来设置大小和颜色；兼容性：网页字体支持所有现代浏览器
